...
function wpm_posts_filter($query)
{
    ...
    if ('wpm-page' == $type && is_admin() && $pagenow == 'edit.php') {
        $taxQuery = array();
...
Приятной работы!"
        ...
        jQuery(function ($) {
            ...
            $(document).on('click', '.add-manual-keys', function () {
                ...
                if(keys_manual == "" || keys_manual == ' '){
                    alert('<?php _e('Сначала Вставьте свои коды доступа.', 'mbl_admin'); ?>');
                    $('#manual-keys').val('');
                    return;
                }

                if(Math.floor(duration_manual) != duration_manual || !$.isNumeric(duration_manual) || duration_manual < 0){
                    alert('<?php _e('Неверное значение периода. Только целые числа, больше нуля.', 'mbl_admin'); ?>');
                    return;
                }
            ...
            $(document).on('click', '.add-keys', function () {
                ...
                if(Math.floor(count) != count || !$.isNumeric(count) || count < 0){
                    alert('<?php _e('Неверное значение количества ключей. Только целые числа, больше нуля.', 'mbl_admin'); ?>');
                    return;
                }

                tb_show("<?php _e('Оставшиеся коды доступа', 'mbl_admin'); ?>", "#TB_inline?width=640&&height=550&inlineId=wpm_popup_box");
            ...
            $(document).on('click', '.show-used', function () {
                ...
                tb_show("<?php _e('Использованные коды доступа', 'mbl_admin'); ?>", "#TB_inline?width=640&&height=550&inlineId=wpm_popup_box");
            ...
            $(document).on('click', '.remove-keys', function () {
                ...
                var units = $(this).attr('units');
                var is_unlimited = $(this).attr('is_unlimited');
                var do_remove = confirm("<?php _e('Вы действительно хотите удалить коды доступа?', 'mbl_admin'); ?>");
                if (do_remove) {
                    $.ajax({
                        type    : 'POST',
                        url     : ajaxurl,
                        ...
                        success : function (data) {
                            if (!data.error) {
                                ...
                                alert('<?php _e('Коды не удалены', 'mbl_admin'); ?>');
                            }
            ...
            copy_keys.on("aftercopy", function(e) {
                $('.wpm-top-popup-nav .message').html('<?php _e('Скопировано!','mbl_admin'); ?>');
            });
    ...
    <tr>
        <th>
            <?php _e('Видимость', 'mbl_admin'); ?>
        </th>
        <td>
            <p><label><input type="checkbox" name="term_meta[hide_for_no_access]" value="hide" <?php if($term_meta['hide_for_no_access'] == 'hide') echo 'checked'; ?> ><?php _e('Скрывать материалы если нет доступа к этому уровню', 'mbl_admin'); ?></label></p>
        </td>
    </tr>
    <tr class="form-field">
            ...
            <?php if (defined('MBLP_VERSION') && class_exists('woocommerce')) : ?>
                <div>
                    <label>
                        ...
                        <?php $products = wc_get_products(['type' => [MBLPProduct::MBL_PRODUCT_TYPE, MBLPProduct::IPR_PRODUCT_TYPE]]); ?>
                    </label>
                    <select name="term_meta[no_access_product_id]">
                        <?php foreach ($products as $product) : ?>
                            <option value="<?php echo $product->id; ?>" <?php echo wpm_array_get($term_meta, 'no_access_product_id') == $product->id ? 'selected' : ''; ?>><?php echo $product->name; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            <?php endif; ?>
                    ...
                    <div class="wpm-tabs wpm-inner-tabs">
                            ...
                            <div class="wpm-tab-content">
                                <div class="wpm-row">
                                    <label class="schedule-days-wrap">
                                        <?php _e('Отправить письмо за', 'mbl_admin'); ?> <input type="number" min="1" size="2" maxlength="2" name="term_meta[letter_1_days]" value="<?php echo $term_meta['letter_1_days'] ?>">  <?php _e('дней до окончания срока', 'mbl_admin'); ?>
                                    </label>
                                </div>
                                <div class="wpm-row">
                                    <label>
                                        <input name="term_meta[letter_1_title]" type="text" class="large-text" value="<?php echo $term_meta['letter_1_title'] ?>" placeholder="<?php _e('Заголовок письма', 'mbl_admin'); ?>">
                                    </label>
                                </div>
                                <div class="wpm-row">
                        ...
                        <div id="tab-2" class="tab">
                            <div class="wpm-tab-content">
                                <div class="wpm-row">
                                    <label class="schedule-days-wrap">
                                        <?php _e('Отправить письмо за', 'mbl_admin'); ?> <input type="number" min="1" size="2" maxlength="2" name="term_meta[letter_2_days]" value="<?php echo $term_meta['letter_2_days'] ?>">  <?php _e(' дней до окончания срока', 'mbl_admin'); ?>
                                    </label>
                                </div>
                                <div class="wpm-row">
                                    <label>
                                        <input name="term_meta[letter_2_title]" type="text" class="large-text" value="<?php echo $term_meta['letter_2_title'] ?>" placeholder="<?php _e('Заголовок письма', 'mbl_admin'); ?>">
                                    </label>
                                </div>
                                <div class="wpm-row">
                        ...
                        <div id="tab-3" class="tab">
                            <div class="wpm-tab-content">
                                <div class="wpm-row">
                                    <label class="schedule-days-wrap">
                                        <?php _e('Отправить письмо за', 'mbl_admin'); ?> <input type="number" min="1" size="2" maxlength="2" name="term_meta[letter_3_days]" value="<?php echo $term_meta['letter_3_days'] ?>">  <?php _e(' дней до окончания срока', 'mbl_admin'); ?>
                                    </label>
                                </div>
                                <div class="wpm-row">
                                    <label>
                                        <input name="term_meta[letter_3_title]" type="text" class="large-text" value="<?php echo $term_meta['letter_3_title'] ?>" placeholder="<?php _e('Заголовок письма', 'mbl_admin'); ?>">
                                    </label>
                                </div>
                                <div class="wpm-row">
                                ...
                                <div class="wpm-tabs wpm-inner-tabs">
                                    ...
                                    <div id="tab-2" class="tab">
                                        <div class="wpm-tab-content">
                                            <div class="wpm-row">
                                                <label>
                                                    <input name="term_meta[mass_keys_title]" type="text" class="large-text" value="<?php echo wpm_array_get($term_meta, 'mass_keys_title', '') !== '' ? wpm_array_get($term_meta, 'mass_keys_title') : $defaultTermMeta['mass_keys_title'] ?>" placeholder="<?php _e('Заголовок письма', 'mbl_admin'); ?>">
                                                </label>
                                            </div>
                                            <div class="wpm-row">
